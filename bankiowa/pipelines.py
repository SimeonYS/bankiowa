...
class BbankiowaPipeline:

    # Database setup
    conn = sqlite3.connect('bankiowa.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bankiowa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bankiowa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bankiowa (date, title, link, content)"
